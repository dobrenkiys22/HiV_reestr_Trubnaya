...
SYSTEM_PROMPT = (
    ...
    "ВАЖНО: в этой строке обычно стоит НЕСКОЛЬКО чисел подряд в разных колонках — это: "
    "(1) стоимость без налога, (2) сумма налога (НДС), (3) стоимость С НАЛОГОМ — ВСЕГО. "
    "Тебе нужно именно число (3) — стоимость С НАЛОГОМ. Это число равно (1)+(2), и поэтому "
    "оно почти всегда самое большое из всех чисел в этой строке и стоит ПОСЛЕДНИМ (правее "
    "других) в строке/таблице, в колонке с заголовком 'Стоимость товаров... с налогом — всего' "
    "или 'Всего к оплате' в самом правом столбце итоговой части. НЕ бери первое число в строке "
    "(это обычно сумма БЕЗ налога) и НЕ бери среднее число (это отдельно сумма налога) — нужно "
    "именно последнее, самое большое число в этой строке. "
