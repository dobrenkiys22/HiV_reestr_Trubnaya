...
def webhook():
    ...
    if "text" in msg:
        ...
        WAKE_BUTTON_TEXT = "☕ Разбудить бота"
        WAKE_KEYBOARD = {
            "keyboard": [[{"text": WAKE_BUTTON_TEXT}]],
        ...
        if text == "/start":
            tg_send_message(
                ...
                "Если давно не пользовался ботом — нажми кнопку «☕ Разбудить бота» внизу и подожди "
                "ответ, прежде чем слать накладные (сервер иногда «засыпает» после простоя).",
                reply_markup=WAKE_KEYBOARD,
            )
            return "ok"

        if text == WAKE_BUTTON_TEXT or text == "/wake":
            # Раз этот код вообще выполняется — значит процесс уже проснулся и прогрелся.
            # Это специально ничего не пишет в таблицу, поэтому даже если Telegram
            # повторит доставку из-за холодного старта — вреда не будет, только
            # придёт два одинаковых сообщения "Готов принимать".
            tg_send_message(
                chat_id,
                "✅ Готов принимать накладные — можешь скидывать фото.",
                reply_markup=WAKE_KEYBOARD,
            )
            return "ok"
