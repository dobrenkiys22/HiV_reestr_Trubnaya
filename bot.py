...
SYSTEM_PROMPT = (
    ...
    "ПРАВИЛО ПОИСКА СУММЫ: ищи строку 'Всего к оплате' / 'Итого' в самом низу товарной таблицы. "
    "ВАЖНО: в этой строке обычно стоит НЕСКОЛЬКО уже готовых, напечатанных чисел подряд в "
    "разных колонках — например: стоимость без налога, сумма налога (НДС), и стоимость С "
    "НАЛОГОМ — ВСЕГО. Тебе нужно именно последнее число в строке (самое правое, в колонке "
    "'Стоимость товаров... с налогом — всего'), и это, как правило, физически самое большое "
    "напечатанное число среди них. "
    "КРИТИЧЕСКИ ВАЖНО: НИКОГДА не складывай, не вычисляй и не пересчитывай числа самостоятельно. "
    "Нужное число уже напечатано на документе готовым — просто найди и прочитай его, не делай "
    "никакой арифметики. Если случайно прочитал не то число — всё равно бери его как есть, "
    "не пытайся компенсировать вычислением. "
    "НЕ бери первое число в строке (это обычно сумма БЕЗ налога) и НЕ бери среднее число (это "
    "отдельно сумма налога) — нужно именно последнее напечатанное число в этой строке. "
    "ЕСЛИ НА ПЕРЕДАННОМ ТЕБЕ ФОТО ЕСТЬ СТРОКА 'ВСЕГО К ОПЛАТЕ' С ЧИСЛАМИ — нужное число "
    "ОБЯЗАТЕЛЬНО присутствует, отказываться его определить нельзя. "
    ...
    "решения о сумме. Если строка 'Всего к оплате' присутствует на фото — используй её число, "
    "даже если рядом написано 'страница 1 из 3' или похожее — это не имеет значения. "
    "Тебе НЕЛЬЗЯ отказываться определить сумму, писать summa:null или объяснять, что 'нужны "
    "другие страницы' — если на фото, которое тебе передали, есть итоговая строка с числом, "
    "указывай это число и ставь uverennost 'high'. Поле summa:null допустимо ТОЛЬКО если на "
    "фото вообще никакой итоговой строки с суммой не видно. "
    "Если в документе несколько кандидатов на итоговую сумму на ОДНОМ И ТОМ ЖЕ фото (например, "
    "промежуточный итог по товару и общий итог документа) — выбирай САМОЕ БОЛЬШОЕ итоговое "
    "число, подписанное как 'Всего к оплате'. "
    "Если тебе передали несколько фото ОДНОГО документа — используй итоговую сумму с "
