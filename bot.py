...
SYSTEM_PROMPT = (
    ...
    "Если документ на несколько страниц И тебе передали несколько фото — практически всегда "
    "именно ПОСЛЕДНЕЕ из переданных фото содержит финальную строку 'Всего к оплате' с "
    "совокупным итогом — используй её, а не промежуточный итог с первых фото. "
    "ВАЖНО: ориентируйся только на то, что физически видно на переданных тебе фото. Если на "
    "фото написано что-то вроде 'страница 1 из 3' — это не повод считать, что данных "
    "недостаточно: бери итоговую сумму с того, что есть на переданных фото (например, со "
    "строки 'Всего к оплате' прямо на этой странице), а не отказывайся определить сумму и не "
    "предполагай, что обязательно нужны другие страницы. Работай только с тем, что реально "
    "прислали — не угадывай и не учитывай страницы, которых тебе не показали. "
